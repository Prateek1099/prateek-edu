...
import { ArrowLeft, CheckCircle2, Clock3, Send, Users } from "lucide-react";
...
export function AssessmentResultsClient({ initialResults }: { initialResults: Results }) {
  ...
  return <div className="mx-auto max-w-6xl space-y-7 pb-10">
    <Link href="/workspace/paper-builder/archive" className={cn(buttonVariants({variant:"ghost",size:"sm"}),"-ml-2")}><ArrowLeft className="size-4"/> Teacher Paper Archive</Link>
    <header><p className="text-sm font-semibold text-primary">Online test results</p><h1 className="mt-1 text-2xl font-bold tracking-tight sm:text-3xl">{results.title}</h1><p className="mt-2 text-sm text-muted-foreground">{results.className} · {results.subjectName} · {results.questionCount} questions · {results.totalMarks} marks</p></header>
    ...
    <section aria-labelledby="student-results-heading"><div className="mb-3"><h2 id="student-results-heading" className="text-lg font-semibold">Student results</h2><p className="mt-1 text-sm text-muted-foreground">Release each graded result when you are ready for that student to see marks and answer review.</p></div>
      <div className="space-y-3 md:hidden">{results.students.map(student=><article key={student.recipientId} className="rounded-2xl border bg-card p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate font-semibold">{student.name||student.email||"Unnamed student"}</h3><p className="truncate text-xs text-muted-foreground">{student.email||"No email"}</p></div><span className="shrink-0 rounded-full bg-muted px-2 py-1 text-xs font-semibold">{labels[student.status]??student.status}</span></div><div className="mt-4 flex items-end justify-between gap-3"><div className="text-sm">{student.awardedMarks===null?<span className="text-muted-foreground">No score yet</span>:<><span className="font-semibold">{student.awardedMarks}/{student.totalMarks}</span><span className="text-muted-foreground"> · {student.percentage}%</span></>}</div>{student.status==="GRADED"&&student.attemptId?<Button size="sm" disabled={pending} onClick={()=>release(student.attemptId!)}>{releasing===student.attemptId?"Releasing…":"Release"}</Button>:null}</div></article>)}</div>
      <div className="hidden overflow-x-auto rounded-2xl border bg-card md:block"><table className="w-full min-w-[760px] text-left text-sm"><thead className="border-b bg-muted/30 text-xs uppercase tracking-wide text-muted-foreground"><tr><th className="px-4 py-3">Student</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Attempt</th><th className="px-4 py-3">Score</th><th className="px-4 py-3">Submitted</th><th className="px-4 py-3 text-right">Action</th></tr></thead><tbody className="divide-y">{results.students.map(student=><tr key={student.recipientId}><td className="px-4 py-3"><p className="font-medium">{student.name||student.email||"Unnamed student"}</p><p className="text-xs text-muted-foreground">{student.email||"No email"}</p></td><td className="px-4 py-3">{labels[student.status]??student.status}</td><td className="px-4 py-3">{student.attemptNumber??"—"}</td><td className="px-4 py-3">{student.awardedMarks===null?"—":`${student.awardedMarks}/${student.totalMarks} (${student.percentage}%)`}</td><td className="px-4 py-3">{student.submittedAt?formatAdminDateTime(student.submittedAt):"—"}</td><td className="px-4 py-3 text-right">{student.status==="GRADED"&&student.attemptId?<Button size="sm" disabled={pending} onClick={()=>release(student.attemptId!)}>{releasing===student.attemptId?"Releasing…":"Release result"}</Button>:student.status==="RELEASED"?<span className="text-xs font-medium text-emerald-700 dark:text-emerald-300">Released</span>:null}</td></tr>)}</tbody></table></div>
    </section>
  </div>;
}
