...
type TopicOption = { id: string; label: string; subjectId: string; importCode: string | null };
...
function ImportDialog({ open, onClose, subjects, topics }: { open: boolean; onClose: () => void; subjects: SubjectOption[]; topics: TopicOption[] }) {
  ...
  const parseCsv = () => setCsvResult(parseBankQuestionCsv(csvText, subjectId, topics.filter((topic) => topic.subjectId === subjectId).map((topic) => ({ id: topic.id, subjectId: topic.subjectId, name: topic.label, importCode: topic.importCode }))));
      ...
      <DialogContent className="max-h-[94vh] overflow-y-auto sm:max-w-6xl">
        ...
        <Tabs defaultValue="csv">
          ...
          <TabsContent value="csv" className="space-y-4 pt-4">
            <div className="rounded-xl border bg-muted/30 p-4 text-sm"><p className="font-medium">Preferred canonical header</p><code className="mt-2 block overflow-x-auto text-xs">ChapterCode,QuestionType,Question,OptionA,OptionB,OptionC,OptionD,Answer,ModelAnswer,Explanation,Difficulty,Marks,Source</code><p className="mt-2 text-xs text-muted-foreground">Use a short topic code such as 101. TopicID remains supported; ChapterID first matches an internal ID, then an import code. Importance is allowed only with an explicit unsupported/deferred warning.</p></div>
            ...
            <Textarea className="min-h-64 font-mono text-xs" value={csvText} onChange={(event) => { setCsvText(event.target.value); setCsvResult(null); }} placeholder={'ChapterCode,QuestionType,Question,OptionA,OptionB,OptionC,OptionD,Answer,ModelAnswer,Explanation,Difficulty,Marks,Source\n101,MCQ,"What is SQL?",A,B,C,D,A,,,easy,1,Textbook'} />
...
function Filter({ value, label, options, onChange, disabled }: { value: string; label: string; options: { value: string; label: string }[]; onChange: (value: string) => void; disabled?: boolean }) {
  return <Select value={value} onValueChange={(next) => onChange(next || "")} disabled={disabled}><SelectTrigger className="w-full" aria-label={label}><SelectValue placeholder={`Select ${label.toLowerCase()}`}>{options.find((option) => option.value === value)?.label}</SelectValue></SelectTrigger><SelectContent>{options.map((option) => <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>)}</SelectContent></Select>;
}
