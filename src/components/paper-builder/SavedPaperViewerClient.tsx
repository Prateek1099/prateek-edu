...
import {
  PaperAnswerKeyDocument,
  PaperQuestionDocument,
} from "@/components/paper-builder/PaperBuilderDocuments";
...
export function SavedPaperViewerClient({
  saved,
  archiveHref,
  ...
  return (
    <div className="space-y-6">
      <div className="paper-builder-screen-only flex flex-wrap items-center justify-between gap-3">
        <Link href={archiveHref} className={buttonVariants({ variant: "outline" })}>
          <ArrowLeft className="size-4" /> {archiveLabel}
        </Link>
        <Badge variant={saved.archivedAt ? "outline" : "secondary"}>
          {saved.archivedAt ? "Archived" : "Active"}
        </Badge>
      </div>
      <section className="paper-builder-screen-only rounded-2xl border bg-card p-5">
        <h1 className="text-2xl font-bold">{saved.name}</h1>
        {saved.description && (
          <p className="mt-2 text-sm text-muted-foreground">{saved.description}</p>
        )}
      ...
      <div className="paper-builder-screen-only flex flex-wrap gap-2">
        ...
        <Button
          type="button"
          variant={preview === "answers" ? "default" : "outline"}
          onClick={() => setPreview("answers")}
        >
          <CheckCircle2 className="size-4" /> Answer key
        </Button>
      </div>
      <div className="paper-builder-screen-only flex flex-wrap gap-2">
        <Button type="button" variant="outline" onClick={() => print("questions")}>
          <Printer className="size-4" /> Print question paper
        </Button>
        <Button type="button" variant="outline" onClick={() => print("answers")}>
          <Printer className="size-4" /> Print answer key
        </Button>
        <Button type="button" onClick={() => print("both")}>
          <Printer className="size-4" /> Print both
        </Button>
      </div>
      <div className="paper-builder-screen-only flex flex-wrap gap-2">
      ...
      <div className={cn(preview !== "questions" && "paper-builder-preview-hidden")}>
        <PaperQuestionDocument paper={saved.paper} />
      </div>
      <div className={cn(preview !== "answers" && "paper-builder-preview-hidden")}>
        <PaperAnswerKeyDocument paper={saved.paper} />
      </div>
    </div>
  );
}
