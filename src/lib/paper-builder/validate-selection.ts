...
export type PaperBuilderValidationAccess = {
...
export async function validatePaperBuilderSelectionForAccess(
  input: PaperValidationInput,
  access: PaperBuilderValidationAccess,
  ...
  try {
    ...
    const ownershipWhere: Prisma.BankQuestionWhereInput =
      access.questionScope.kind === "global-only"
        ? { workspaceId: null }
        : {
            OR: [
              { workspaceId: null },
    ...
    const records = await prisma.bankQuestion.findMany({
      where: {
        ...
        topicId: { in: topicIds },
        questionType: { in: [...access.allowedQuestionTypes] },
        ...ownershipWhere,
      },
      select: {
        id: true,
    ...
    if (records.length !== selectedIds.length) {
      return {
        success: false,
        error: "One or more questions are missing or outside the permitted academic and workspace scope.",
      };
    }
